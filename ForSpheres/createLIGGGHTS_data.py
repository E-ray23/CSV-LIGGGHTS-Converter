...
def create_Liggghts_file(atom_section_df, tot_number_of_particles, radius, density):
    file_save_path = "convertingZone/spheres_positions.data"
    atom_types = 1
    ...
    if os.path.exists(file_save_path):
        ...
        with open(file_save_path, 'w') as fp:
            ...
            fp.write("# LIGGGHTS data file generated from CSV\n\n")
            fp.write("\t\t\t\t\t#### HEADER\n")
            ...
            fp.write("\t\t\t\t\t#### BODY\n")
            fp.write("Masses\n")
            fp.write("1 1.0 \t\t\t\t\t\t # Mass for atom type 1\n\n")
            ...
            for index, row in atom_section_df.iterrows():
                if index < tot_number_of_particles: #Ensure not to create an empty line of Atoms-Section
                    fp.write(f"{index} {atom_types} {row['Points_0']} {row['Points_1']} {row['Points_2']} {radius} {density}\n")
                else:
                    fp.write(f"{index} {atom_types} {row['Points_0']} {row['Points_1']} {row['Points_2']} {radius} {density}")
